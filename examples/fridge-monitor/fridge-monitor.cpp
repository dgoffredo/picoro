...
picoro::Coroutine<void> monitor_sensor(async_context_t* ctx, picoro::dht22::Driver* driver, PIO pio,
  ...
  for (;;) {
    ...
    if (Sensor::Result rc = co_await sensor.measure(&celsius, &humidity_percent)) {
      std::printf("{\"error\": \"%s\"}\n", Sensor::describe(rc));
      sensor.reset();
      continue;
    }
...
int main() {
  ...
  picoro::run_event_loop(ctx, monitor_sensor(ctx, &driver, pio0, 15, "top shelf"),
